...
class GroundTruthLabelCreator:
  def __init__(self, image_size, anchors_by_stage, stage_num_cells, pos_iou_threshold, neg_iou_threshold,
               grid_expand_n_at_borders=0):
    self.image_size = image_size
    self.anchors_by_stage = anchors_by_stage
    self.pos_iou_threshold = pos_iou_threshold
    self.neg_iou_threshold = neg_iou_threshold
    self.grid_expand_n_at_borders = grid_expand_n_at_borders
    self.stage_num_cells = [nc + (2 * grid_expand_n_at_borders) for nc in stage_num_cells]
  ...
  def initialize_anchors(self):
    ...
    for stage_idx, stage_anchors in enumerate(self.anchors_by_stage):
      ...
      for anchor_idx, anchor in enumerate(stage_anchors):
        aw, ah = anchor
        for col in range(-self.grid_expand_n_at_borders, num_cells):
          for row in range(-self.grid_expand_n_at_borders, num_cells):
  ...
  def create_gt_masks(self, batch_gt_boxes):
    ...
    for stage_idx, num_cells in enumerate(self.stage_num_cells):
      ...
      for anchor_idx in range(0, len(self.anchors_by_stage[stage_idx])):
        ...
        anchor_num_matches_by_stage[stage_idx].append(
          np.zeros([batch_size, num_cells, num_cells, 1]))
    ...
    batch_gt_boxes_matches = [[] for _ in range(0, batch_size)]
    for batch_idx in range(0, batch_size):
      ...
      for gt_idx, abs_gt_box in enumerate(abs_gt_boxes):
        box_ious = ious[gt_idx]
        positive_anchor_indices = np.where(box_ious > self.pos_iou_threshold)[0]

        batch_gt_boxes_matches[batch_idx].append(positive_anchor_indices)
        ...
        for idx in positive_anchor_indices:
          if not idx in anchor_box_matches:
            anchor_box_matches[idx] = []
          anchor_box_matches[idx].append([abs_gt_box, box_ious[idx]])
      ...
      for idx in anchor_box_matches.keys():
        matches = anchor_box_matches[idx]
        abs_gt_boxes = [m[0] for m in matches]
        box_ious = [m[1] for m in matches]
        abs_gt_box = abs_gt_boxes[np.argmax(box_ious)]
        ...
        anchor_num_matches_by_stage[stage_idx][anchor_idx][batch_idx, col, row, :] = len(matches)
    ...
    masks = {
      ...
      "batch_gt_boxes_matches": batch_gt_boxes_matches,
      "anchor_num_matches_by_stage": anchor_num_matches_by_stage
    }
    return masks
