...
def calculate_stats(inputs, score_thresh=0.5, iou_threshs=[0.5]):
  ...
  gt_pos_by_stage = [np.where(inputs["pos_anchors_masks_by_stage"][s] != 0) for s in range(0, num_stages)]
  num_gt_anchors_by_stage = [gt_pos_by_stage[s][0].shape[0] for s in range(0, num_stages)]
  num_tps_score_by_stage = [np.where(inputs["batch_scores_by_stage"][s][gt_pos_by_stage[s]] > score_thresh)[0].shape[0]
