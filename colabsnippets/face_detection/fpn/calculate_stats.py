...
def calculate_stats(inputs, score_thresh = 0.5, iou_threshs = [0.5]):
  ...
  return {
    "num_tps": num_tps,
    "num_matches": [np.sum([len(good_boxes) for good_boxes in batch_good_boxes[i]]) for i in range (0, len(iou_threshs))],
    "num_tps_score": num_tps_score,
    "num_preds": num_preds,
    "num_gt_boxes": num_gt_boxes,
    "num_gt_anchors": num_gt_anchors,
